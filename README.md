...
[![Tests](https://github.com/csgt/utils/actions/workflows/tests.yml/badge.svg?branch=7.0)](https://github.com/csgt/utils/actions/workflows/tests.yml)
...
One branch per major version — every version is in active use by projects pinned to it, so fixes are applied to the branch of the affected version. This is the 7.0 line (Laravel CSGT using laravel ui; 6.0 is the same without laravel ui):

```bash
composer require csgt/utils:^7.0
```
...
Work targets the branch of the affected version (this is 7.0). The test suite runs the scaffolding commands against a real Laravel skeleton via `orchestra/testbench`:
