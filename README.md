...
[![Tests](https://github.com/csgt/utils/actions/workflows/tests.yml/badge.svg?branch=5.0)](https://github.com/csgt/utils/actions/workflows/tests.yml)
...
One branch per major version — every version is in active use by projects pinned to it, so fixes are applied to the branch of the affected version. This is the 5.0 line:

```bash
composer require csgt/utils:^5.0
```
...
Work targets the branch of the affected version (this is 5.0). The test suite runs the scaffolding commands against a real Laravel skeleton via `orchestra/testbench`:
